...
import { GripVertical, ArrowLeft, Layers, UserPlus, Check, X, Download, Upload, ChevronRight, ChevronDown, Copy, ThumbsUp, ThumbsDown } from "lucide-react";
import {
  createColumnHelper,
  flexRender,
  getCoreRowModel,
...
export default function ReviewArtifactPanel({
  ...
  // Cell preview popover state
  const [selectedCell, setSelectedCell] = React.useState<{ rowId: number; columnId: string; content: string; fileName: string } | null>(null);
  const [cellPopoverPosition, setCellPopoverPosition] = React.useState({ top: 0, left: 0 });
  
  const handleCellClick = React.useCallback((rowId: number, columnId: string, content: string, fileName: string, cellElement: HTMLElement) => {
    if (!content) return;
    ...
    // Try placing to the right of the cell
    if (rect.right + 4 + popoverWidth <= window.innerWidth - 8) {
      left = rect.right + 4;
    } else {
      // Place to the left of the cell
    ...
    setSelectedCell({ rowId, columnId, content, fileName });
  }, []);
  ...
  const [preImproveQuestion, setPreImproveQuestion] = React.useState<string | null>(null);
  ...
  const handleImprovePrompt = () => {
    setPreImproveQuestion(columnQuestion);
    setIsImprovingPrompt(true);
    
    const improvedText = `${columnQuestion.replace(/\?$/, '')}? Please include the exact date as stated in the document, noting any amendments or restated dates. If multiple signing dates exist (e.g., for different counterparties), list each date alongside the corresponding signatory. Flag any undated signature blocks.`;
    
    setTimeout(() => {
      handleQuestionChange(improvedText);
      setIsImprovingPrompt(false);
    }, 1500);
  };
  
  const handleUndoImprove = () => {
    if (preImproveQuestion !== null) {
      handleQuestionChange(preImproveQuestion);
      setPreImproveQuestion(null);
    }
  };
  ...
  return (
    <>
                        ...
                        <td
                          ...
                          style={{ 
                            ...
                            boxShadow: isCellSelected 
                              ? 'inset 0 0 0 1.5px var(--fg-base)' 
                              : cell.column.id === 'fileName' && isScrolledHorizontally 
                                ? '4px 0 8px -2px rgba(0,0,0,0.06)' 
                                : undefined,
                            borderRadius: isCellSelected ? '6px' : undefined,
                          }}
                          onClick={isContentCell && !isFolder ? (e) => {
                            e.stopPropagation();
                            const cellContent = (() => {
                              const dynamicData = cellDataRef.current[cell.column.id]?.[row.original.id];
                              if (dynamicData) return dynamicData.response || '';
                              const val = row.original[cell.column.id as keyof Document];
                              return typeof val === 'string' ? val : '';
                            })();
                            handleCellClick(row.original.id, cell.column.id, cellContent, row.original.fileName, e.currentTarget);
                          } : undefined}
                        >
                          {flexRender(
      ...
      {selectedCell && (
        ...
        >
          <div 
            className="fixed bg-bg-subtle rounded-[12px] overflow-hidden"
            style={{ 
              width: '380px',
              top: cellPopoverPosition.top,
              left: cellPopoverPosition.left,
              boxShadow: '0 0 0 1px rgba(0,0,0,0.08), 0 4px 12px rgba(0,0,0,0.06), 0 12px 32px rgba(0,0,0,0.08)',
            }}
            onClick={(e) => e.stopPropagation()}
          >
            <div className="bg-bg-base border border-border-base rounded-[10px] overflow-hidden flex flex-col">
              {/* Summary section */}
              <div className="border-b border-border-base pb-2 pt-3">
                <div className="flex items-center justify-between px-3">
                  <p className="text-fg-muted font-medium" style={{ fontSize: '12px', lineHeight: '16px' }}>Summary</p>
                  <button className="w-6 h-6 flex items-center justify-center rounded-[6px] hover:bg-bg-subtle transition-colors text-fg-subtle">
                    <Copy size={14} />
                  </button>
                </div>
                <div className="px-1">
                  <div className="px-2 py-2 rounded-[6px]">
                    <p className="text-fg-base" style={{ fontSize: '12px', lineHeight: '16px' }}>
              ...
              {/* Additional context */}
              <div className="border-b border-border-base p-3 flex flex-col gap-1">
                <div className="flex items-center justify-between">
                  <p className="text-fg-muted font-medium" style={{ fontSize: '12px', lineHeight: '16px' }}>Additional context</p>
                  <button className="w-6 h-6 flex items-center justify-center rounded-[6px] hover:bg-bg-subtle transition-colors text-fg-subtle">
                    <Copy size={14} />
                  </button>
                </div>
                <p className="text-fg-base" style={{ fontSize: '12px', lineHeight: '16px' }}>
                  No additional context available for this cell.
                </p>
              </div>

              {/* Confidence score */}
              <div className="border-b border-border-base p-3 flex flex-col" style={{ gap: '8px' }}>
                <div className="flex items-center justify-between">
                  <p className="text-fg-muted font-medium" style={{ fontSize: '12px', lineHeight: '16px' }}>Confidence score</p>
                  <span
                    className="inline-flex items-center justify-center font-medium rounded-[4px] px-1"
                    style={{ fontSize: '12px', lineHeight: '16px', backgroundColor: 'var(--ui-warning-bg, #ffedd5)', color: 'var(--ui-warning-fg, #9a3412)' }}
                  >
                    Medium
                  </span>
                </div>
                <p className="text-fg-base" style={{ fontSize: '12px', lineHeight: '16px' }}>
                  The underlying mechanism are well-documented internally, but I could not find an authoritative source addressing the specific 240-page claim or a ready-made customer-facing explanation.
                </p>
              </div>

              {/* Source */}
              <div className="border-b border-border-base p-3 flex flex-col gap-1">
                <p className="text-fg-muted font-medium" style={{ fontSize: '12px', lineHeight: '16px' }}>Source</p>
                <div className="flex items-center justify-between">
                  <p className="text-fg-base underline" style={{ fontSize: '12px', lineHeight: '16px' }}>
                    {selectedCell.fileName}
                  </p>
                  <div className="flex gap-[2px]">
                    <span className="bg-bg-subtle rounded-[2px] px-[2px] min-w-[14px] text-center font-medium" style={{ fontSize: '10px', lineHeight: '14px' }}>1</span>
                    <span className="bg-bg-subtle rounded-[2px] px-[2px] min-w-[14px] text-center font-medium" style={{ fontSize: '10px', lineHeight: '14px' }}>2</span>
                  </div>
                </div>
              </div>

              {/* Action buttons */}
              <div className="border-b border-border-base py-1">
                <div className="px-1">
                  <button className="w-full flex items-center justify-between px-2 rounded-[6px] hover:bg-bg-subtle transition-colors" style={{ height: '32px' }}>
                    <span className="text-fg-muted font-medium" style={{ fontSize: '12px', lineHeight: '16px' }}>Assign to</span>
                    <div className="flex items-center gap-1">
                      <div className="w-4 h-4 rounded-full bg-fg-subtle flex items-center justify-center">
                        <span className="text-white font-medium" style={{ fontSize: '9px' }}>H</span>
                      </div>
                      <span className="text-fg-base" style={{ fontSize: '13px', lineHeight: '20px' }}>Harvey Specter</span>
                      <ChevronRight size={14} className="text-fg-muted" />
                    </div>
                  </button>
                </div>
                <div className="px-1">
                  <button className="w-full flex items-center justify-between px-2 rounded-[6px] hover:bg-bg-subtle transition-colors" style={{ height: '32px' }}>
                    <span className="text-fg-muted font-medium" style={{ fontSize: '12px', lineHeight: '16px' }}>Verification status</span>
                    <div className="flex items-center gap-1">
                      <SvgIcon src="/central_icons/Flag - Filled.svg" alt="" width={14} height={14} className="text-ui-danger-fg" />
                      <span className="text-fg-base" style={{ fontSize: '13px', lineHeight: '20px' }}>Flagged for review</span>
                      <ChevronRight size={14} className="text-fg-muted" />
                    </div>
                  </button>
                </div>
              ...
              {/* Flagged by */}
              <div className="border-b border-border-base p-3 flex flex-col gap-1.5">
                <p className="text-fg-muted font-medium" style={{ fontSize: '12px', lineHeight: '16px' }}>Flagged by</p>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1">
                    <div className="w-4 h-4 rounded-full bg-fg-subtle flex items-center justify-center">
                      <span className="text-white font-medium" style={{ fontSize: '9px' }}>A</span>
                    </div>
                    <span className="text-fg-base" style={{ fontSize: '12px', lineHeight: '16px' }}>aaron@acme.com</span>
                  </div>
                  <span className="text-fg-muted" style={{ fontSize: '10px', lineHeight: '14px' }}>1 minute ago</span>
                </div>
              </div>

              {/* Comment textarea */}
              <div className="p-1">
                <div className="bg-bg-subtle rounded-[8px] relative" style={{ height: '120px' }}>
                  <textarea
                    className="w-full h-full bg-transparent resize-none outline-none p-2 text-fg-base"
                    style={{ fontSize: '12px', lineHeight: '16px' }}
                    defaultValue="Termination clause misses the 30-day notice/cure period, exclusivity is marked as &quot;exclusive&quot; but the contract says &quot;preferred,&quot; and late fees (1.5%/mo) weren't captured. Let me know if I'm off."
                  />
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between px-3 py-1.5">
              <span className="text-fg-muted" style={{ fontSize: '10px', lineHeight: '14px' }}>How would you rate the response?</span>
              <div className="flex items-center">
                <button className="w-6 h-6 flex items-center justify-center rounded-[6px] hover:bg-bg-subtle-hover transition-colors text-fg-subtle">
                  <ThumbsUp size={14} />
                </button>
                <button className="w-6 h-6 flex items-center justify-center rounded-[6px] hover:bg-bg-subtle-hover transition-colors text-fg-subtle">
                  <ThumbsDown size={14} />
        ...
        >
          ...
          >
            ...
            <div style={{ padding: '2px 4px 4px' }}>
              <div className="relative bg-bg-subtle rounded-[8px] flex flex-col">
                <div className="relative flex-1" style={{ minHeight: '56px', maxHeight: '140px' }}>
                  <div className="overflow-y-auto p-[10px] h-full" style={{ minHeight: '56px', maxHeight: '140px' }}>
                    <textarea
                      value={columnQuestion}
                      onChange={(e) => { setPreImproveQuestion(null); handleQuestionChange(e.target.value); }}
                      placeholder="What is the signing date of this agreement?"
                      className="w-full bg-transparent text-fg-base placeholder:text-fg-muted resize-none outline-none"
                      style={{ 
                        minHeight: '36px',
                        height: 'auto',
                        fontSize: '14px',
                  ...
                  <div className="absolute bottom-0 left-0 right-0 h-4 bg-gradient-to-t from-bg-subtle to-transparent pointer-events-none rounded-b-[8px]" />
                </div>
                <div className="border-t border-border-base px-[10px] py-[8px]">
                  <p className="text-fg-muted font-medium" style={{ fontSize: '12px', lineHeight: '16px' }}>
                    Use @ to reference columns
                  </p>
                </div>
                {isImprovingPrompt && (
                  <div
                    className="absolute inset-0 animate-grouping-shimmer pointer-events-none"
                    style={{ borderRadius: '8px' }}
                  />
                )}
              </div>
            ...
            {/* Improve prompt / Undo button — visible after typing */}
            {hasStartedTyping && (
              <div style={{ padding: '2px 4px 4px' }}>
                {preImproveQuestion !== null && !isImprovingPrompt ? (
                  <Button
                    variant="outline"
                    size="medium"
                  ...
                  >
                    <SvgIcon src="/central_icons/Undo.svg" alt="" width={14} height={14} />
                    <span>Undo</span>
                  </Button>
                ) : (
                  <Button
                    ...
                    disabled={isImprovingPrompt}
                  >
                    <SvgIcon src="/central_icons/Magic Broom.svg" alt="" width={14} height={14} />
            ...
            {hasStartedTyping && (
              <>
                ...
                <div className="flex flex-col" style={{ padding: '2px 0', gap: '2px' }}>
                  ...
                  {/* Type selector */}
                  <div style={{ padding: '0 4px' }}>
                    <button className="w-full flex items-center justify-between px-2 rounded-[8px] hover:bg-bg-subtle-hover transition-colors" style={{ height: '28px' }}>
                  ...
                  <div style={{ padding: '0 4px' }}>
                    <DropdownMenu open={modelDropdownOpen} onOpenChange={setModelDropdownOpen}>
                      <DropdownMenuTrigger asChild>
                        <button className={`w-full flex items-center justify-between px-2 rounded-[8px] hover:bg-bg-subtle-hover transition-colors ${modelDropdownOpen ? 'bg-bg-subtle-hover' : ''}`} style={{ height: '28px' }}>
