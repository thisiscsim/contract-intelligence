...
export default function ConfigurationDrawer({ 
  ...
  const [viewMode, setViewMode] = useState<"grid" | "list">(isExpanded ? "grid" : "list");
  ...
  const renderSourceGridCard = (title: string, icon: string, color: string, key: string) => {
    ...
    const iconSrc = icon === "vault" ? "/privateFolderIcon.svg" : icon === "knowledge" ? "/knowledgeBaseIcon.svg" : icon === "folder" ? "/folderIcon.svg" : icon === "xlsx" ? "/xls.svg" : "/msword.svg";
    return (
      ...
      <div className="flex items-center h-8 px-2 -mx-2 rounded-md hover:bg-bg-subtle cursor-pointer" onClick={onToggle}>
        <div className="flex items-center h-full flex-shrink-0">
          ...
          <div className="flex-shrink-0 flex items-center justify-center" style={{ width: 20, height: 16 }}>
            <img src={icon === "folder" ? "/folderIcon.svg" : "/msword.svg"} alt="" className="w-4 h-4" />
          </div>
        </div>
        {/* Label */}
  ...
  const renderSourcesGrid = () => {
    const gridSources = customSources 
      ? customSources.map((s) => ({ title: s.title, icon: s.icon, color: "#CCCAC6" }))
      : defaultVaultSources;
    return (
  ...
  const overviewContent = (
    <div className="space-y-0">
      ...
      {/* Data Shelf */}
      <div className="-mx-4 px-4 pt-0 relative">
        <div className="flex items-center justify-between h-[36px]">
          <span className="text-xs font-medium text-fg-base leading-[20px]">Data</span>
      ...
      {/* Agents Shelf — temporarily hidden */}

      {/* Tools Shelf */}
      <div className="border-t border-border-base -mx-4 px-4 pt-2 pb-5">
        <div className="flex items-center justify-between h-[44px]">
          <span className="text-xs font-medium text-fg-base leading-[20px]">Tools</span>
          <button className="h-[24px] px-2 text-xs font-medium text-fg-subtle hover:text-fg-base hover:bg-bg-subtle rounded-[6px] transition-colors leading-[16px]">
            View all
          </button>
        </div>
        <div className={cn("grid gap-3", isNarrow ? "grid-cols-1" : "grid-cols-2")}>
          {[
            { title: "Draft LP response memo", description: "Generate individually tailored response memos", type: "Draft", steps: "5 steps" },
            { title: "Run MFN cascade analysis", description: "Analyze MFN election exposure across tiers", type: "Output", steps: "4 steps" },
            { title: "Draft side letter", description: "Generate execution-ready side letters", type: "Draft", steps: "6 steps" },
            { title: "Generate redline comparison", description: "Compare LPA versions with tracked changes", type: "Output", steps: "3 steps" },
            { title: "Review LP comment letter", description: "Extract and classify LP comments by section", type: "Review", steps: "4 steps" },
            { title: "Draft board resolutions", description: "Create corporate board meeting resolutions", type: "Draft", steps: "6 steps" },
          ].map((tool, index) => (
            <button key={`tool-${index}`} className="p-4 border border-border-base rounded-lg hover:border-border-strong transition-all text-left overflow-hidden cursor-pointer">
              <p className="text-sm font-medium text-fg-base mb-1 truncate">{tool.title}</p>
              <p className="text-xs text-fg-muted mb-4 truncate">{tool.description}</p>
              <div className="flex items-center gap-1 text-fg-muted">
                <span className="text-xs">{tool.type}</span>
                <span className="text-xs">•</span>
                <span className="text-xs">{tool.steps}</span>
              </div>
            </button>
          ))}
        </div>
      ...
      {customArtifacts && customArtifacts.length > 0 ? (
        ...
        <div className="relative overflow-hidden">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-end pb-6">
            <p className="text-sm font-medium text-fg-base mb-1">No work product</p>
